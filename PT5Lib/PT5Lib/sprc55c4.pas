...
procedure SPrc55Cn1(Name, Key: string);
procedure SPrc55Cn2(Name, Key: string);
procedure SPrc55Cn3(Name, Key: string);
procedure SPrc55Cn4(Name, Key: string);
procedure SPrc55Cn5(Name, Key: string);
procedure SPrc55Cn6(Name, Key: string);
procedure SPrc55Cn7(Name, Key: string);
procedure SPrc55Cn8(Name, Key: string);
procedure SPrc55Cn9(Name, Key: string);
procedure SPrc55Cn10(Name, Key: string);
procedure SPrc55Cn11(Name, Key: string);
procedure SPrc55Cn12(Name, Key: string);
procedure SPrc55Cn13(Name, Key: string);
procedure SPrc55Cn14(Name, Key: string);
procedure SPrc55Cn15(Name, Key: string);
procedure SPrc55Cn16(Name, Key: string);
procedure SPrc55Cn17(Name, Key: string);
procedure SPrc55Cn18(Name, Key: string);
procedure SPrc55Cn19(Name, Key: string);
procedure SPrc55Cn20(Name, Key: string);
procedure SPrc55Cn21(Name, Key: string);
procedure SPrc55Cn22(Name, Key: string);
procedure SPrc55Cn23(Name, Key: string);
procedure SPrc55Cn24(Name, Key: string);
procedure SPrc55Cn25(Name, Key: string);
procedure SPrc55Cn26(Name, Key: string);
procedure SPrc55Cn27(Name, Key: string);
procedure SPrc55Cn28(Name, Key: string);
procedure SPrc55Cn29(Name, Key: string);
procedure SPrc55Cn30(Name, Key: string);
procedure SPrc55Cn31(Name, Key: string);
procedure SPrc55Cn32(Name, Key: string);
procedure SPrc55Cn33(Name, Key: string);
procedure SPrc55Cn34(Name, Key: string);
procedure SPrc55Cn35(Name, Key: string);
procedure SPrc55Cn36(Name, Key: string);
procedure SPrc55Cn37(Name, Key: string);
procedure SPrc55Cn38(Name, Key: string);
procedure SPrc55Cn39(Name, Key: string);
procedure SPrc55Cn40(Name, Key: string);
procedure SPrc55Cn41(Name, Key: string);
procedure SPrc55Cn42(Name, Key: string);
procedure SPrc55Cn43(Name, Key: string);
procedure SPrc55Cn44(Name, Key: string);
procedure SPrc55Cn45(Name, Key: string);
procedure SPrc55Cn46(Name, Key: string);
procedure SPrc55Cn47(Name, Key: string);
procedure SPrc55Cn48(Name, Key: string);
procedure SPrc55Cn49(Name, Key: string);
procedure SPrc55Cn50(Name, Key: string);
procedure SPrc55Cn51(Name, Key: string);
procedure SPrc55Cn52(Name, Key: string);
procedure SPrc55Cn53(Name, Key: string);
procedure SPrc55Cn54(Name, Key: string);
procedure SPrc55Cn55(Name, Key: string);
...
taskText('Write a\ procedure PowerA3({A},\ {B}) that computes the third degree',0,2);
taskText('of a\ real number~{A} and assigns the result to a\ real variable~{B}',0,3);
taskText('({A}~is an input parameter, {B}~is an output parameter).',0,4);
taskText('Using this procedure, find the third degree of five given real numbers.',0,5);
(*
*)
(*==*)
for i := 1 to 5 do
...
taskText('Write a\ procedure PowerA234({A}, {B}, {C}, {D}) that computes the second, the third,',0,1);
taskText('and the fourth degrees of a\ real number~{A} and assigns the results',0,2);
taskText('to real variables~{B}, {C}, and~{D} respectively ({A}~is an input parameter,',0,3);
taskText('{B},~{C}, {D}~are output parameters). Using this procedure, find the second,',0,4);
taskText('the third, and the fourth degrees of five given real numbers.',0,5);
(*
*)
(*==*)
for i := 1 to 5 do
...
taskText('Write a\ procedure Mean({X},\;{Y},\;{AMean},\;{GMean}) that computes the \Iarithmetical mean\i',0,1);
taskText('{AMean}~=~({X}+{Y})/2 and the \Igeometrical mean\i {GMean}~=~\R{X}\*{Y}\r of two positive',0,2);
taskText('numbers~{X} and~{Y} ({X} and~{Y} are input parameters, {AMean} and~{GMean} are output',0,3);
taskText('parameters; all parameters are the real-valued ones). Using this procedure,',0,4);
taskText('find the arithmetical mean and the geometrical mean of pairs ({A},\;{B}),',0,5);
taskText('({A},\;{C}), ({A},\;{D}) provided that real numbers~{A},~{B}, {C},~{D} are given.',0,0);
(*
*)
(*==*)
...
taskText('Write a\ procedure TrianglePS({a}, {P},\ {S}) that computes the perimeter~{P}~=~3\*{a}',0,1);
taskText('and the area~{S}~=~{a}^2\*\R3\r/4 of an equilateral triangle with the side~{a}',0,2);
taskText('({a}~is an input parameter, {P} and~{S} are output parameters, all parameters',0,3);
taskText('are the real-valued ones). Using this procedure, find the perimeters and',0,4);
taskText('the areas of three triangles with the given lengths of the sides.',0,5);
(*
*)
for i := 1 to 3 do
begin
...
taskText('Write a procedure RectPS({x}_1,\;{y}_1,\;{x}_2,\;{y}_2,\;{P},\;{S}) that computes the perimeter~{P}',0,1);
taskText('and the area~{S} of a rectangle whose opposite vertices have coordinates',0,2);
taskText('({x}_1,\;{y}_1) and ({x}_2,\;{y}_2) and sides are parallel to coordinate axes ({x}_1, {y}_1,',0,3);
taskText('{x}_2, {y}_2 are input parameters, {P} and~{S} are output parameters, all parameters',0,4);
taskText('are the real-valued ones). Using this procedure, find the perimeters',0,5);
taskText('and the areas of three rectangles with the given opposite vertices.',0,0);
(*
*)
(*==*)
for i := 1 to 3 do
...
taskText('Write a\ procedure DigitCountSum({K}, {C},\ {S}) that finds the amount~{C} of digits',0,1);
taskText('in the decimal representation of a\ positive integer~{K} and also the sum~{S}',0,2);
taskText('of its digits ({K}~is an input parameter, {C} and~{S} are output parameters,',0,3);
taskText('all parameters are the integer ones). Using this procedure, find the amount',0,4);
taskText('and the sum of digits for each of five given integers.',0,5);
(*
*)
(*==*)
for i := 1 to 5 do
...
taskText('Write a\ procedure InvDigits({K}) that inverts the order of digits',0,2);
taskText('of a positive integer~{K} ({K}~is an input and output integer parameter).',0,3);
taskText('Using this procedure, invert the order of digits',0,4);
...
start(Name,Topic1,Author,Key,36{level1});(*DEMO*)
s1 := 'Right';
taskText('Write a\ procedure AddRightDigit({D}, {K}) that adds a digit~{D} to the right side',0,1);
taskText('of the decimal representation of a positive integer~{K} ({D}~is an input integer',0,2);
taskText('parameter with the value in the range~0 to~9, {K}~is an input and output integer',0,3);
taskText('parameter). Having input an integer~{K} and two one-digit numbers~{D}_1, {D}_2 and',0,4);
taskText('using two calls of this procedure, sequentially add the given digits~{D}_1, {D}_2',0,5);
...
start(Name,Topic1,Author,Key,36{level1});
s1 := 'Left';
taskText('Write a\ procedure AddLeftDigit({D}, {K}) that adds a digit~{D} to the left side',0,1);
taskText('of the decimal representation of a positive integer~{K} ({D}~is an input integer',0,2);
taskText('parameter with the value in the range~0 to~9, {K}~is an input and output integer',0,3);
taskText('parameter). Having input an integer~{K} and two one-digit numbers~{D}_1, {D}_2 and',0,4);
taskText('using two calls of this procedure, sequentially add the given digits~{D}_1, {D}_2',0,5);
...
start(Name,Topic1,Author,Key,36{level1});(*DEMO*)
getVariant(2,0,0,0,v1,v2,v3);
taskText('Write a\ procedure Swap({X}, {Y}) that exchanges the values',0,1);
taskText('of variables~{X} and~{Y} ({X} and~{Y} are input and output real-valued parameters).',0,2);
taskText('Having input integers~{A}, {B}, {C},~{D} and using three calls of this procedure,',0,3);
taskText('sequentially exchange the values of the pairs {A} and~{B}, {C} and~{D}, {B} and~{C}.',0,4);
taskText('Output the new values of~{A},~{B}, {C},~{D}.',0,5);
(*
*)
x1 := 9.98 * Random;
x2 := 9.98 * Random;
...
start(Name,Topic1,Author,Key,36{level1});
getVariant(4,0,0,0,v1,v2,v3);
taskText('Write a\ procedure Minmax({X}, {Y}) that exchanges, if necessary,',0,1);
taskText('the values of two variables~{X} and~{Y} so that {X}~\l~{Y}',0,2);
taskText('({X} and~{Y} are input and output real-valued parameters).',0,3);
taskText('Using four calls of this procedure, find the minimal value',0,4);
taskText('and the maximal value among given real numbers~{A},~{B}, {C},~{D}.',0,5);
(*
*)
(*==*)
min := 100; max := -100;
...
begin
s1 := 'Inc';
taskText('Write a\ procedure SortInc3({A}, {B},~{C}) that interchanges, if necessary,',0,1);
taskText('the values of three variables~{A}, {B},~{C} so that {A}~\l~{B}~\l~{C}',0,2);
taskText('({A},~{B}, {C} are input and output real-valued parameters).',0,3);
taskText('Using this procedure, sort each of two given triples of real numbers',0,4);
taskText('({A}_1, {B}_1, {C}_1) and ({A}_2, {B}_2, {C}_2) in ascending order.',0,5);
(*
*)
end
else
begin
s1 := 'Dec';
taskText('Write a\ procedure SortDec3({A}, {B},~{C}) that interchanges, if necessary,',0,1);
taskText('the values of three variables~{A}, {B},~{C} so that {A}~\g~{B}~\g~{C}',0,2);
taskText('({A},~{B}, {C} are input and output real-valued parameters).',0,3);
taskText('Using this procedure, sort each of two given triples of real numbers',0,4);
taskText('({A}_1, {B}_1, {C}_1) and ({A}_2, {B}_2, {C}_2) in descending order.',0,5);
(*
*)
end;
(*==*)
...
s1 := 'Right';
s2 := '';
taskText('Write a procedure ShiftRight3({A}, {B},~{C}) that performs a \Iright cyclic shift\i',0,1);
taskText('by assigning the initial values of~{A}, {B},~{C} to variables~{B}, {C},~{A} respectively',0,2);
taskText('({A}, {B},~{C} are input and output real-valued parameters). Using this procedure,',0,3);
taskText('perform the right cyclic shift for each of two given triples',0,4);
taskText('of real numbers: ({A}_1, {B}_1, {C}_1) and ({A}_2, {B}_2, {C}_2).',0,5);
(*
*)
end
else
...
s1 := 'Left';
s2 := ' ';
taskText('Write a procedure ShiftLeft3({A}, {B},~{C}) that performs a \Ileft cyclic shift\i',0,1);
taskText('by assigning the initial values of~{A}, {B},~{C} to variables~{C}, {A},~{B} respectively',0,2);
taskText('({A}, {B},~{C} are input and output real-valued parameters). Using this procedure,',0,3);
taskText('perform the left cyclic shift for each of two given triples',0,4);
taskText('of real numbers: ({A}_1, {B}_1, {C}_1) and ({A}_2, {B}_2, {C}_2).',0,5);
(*
*)
end;
...
start(Name,Topic3,Author,Key,36{level1});
getVariant(2,0,0,0,v1,v2,v3);
taskText('Write a\ procedure TimeToHMS({T},\ {H}, {M},\ {S}) that converts a\ time interval~{T}',0,1);
taskText('(in seconds) into the \<hours~{H}, minutes~{M}, seconds~{S}\> format',0,2);
taskText('({T} is an input integer parameter, {H},~{M}, {S} are output integer parameters). ',0,3);
taskText('Using this procedure, find the amount of hours, minutes and seconds',0,4);
...
start(Name,Topic3,Author,Key,36{level1});
getVariant(2,0,0,0,v1,v2,v3);
taskText('Write a\ procedure IncTime({H},\;{M},\;{S},\;{T}) that increases a time interval',0,1);
taskText('in hours~{H}, minutes~{M}, seconds~{S} on {T}~seconds ({H}, {M}, {S} are input and output',0,2);
taskText('positive integer parameters, {T}~is an input positive integer parameter).',0,3);
taskText('Having input hours~{H}, minutes~{M}, seconds~{S} (as integers) and an integer~{T}',0,4);
taskText('and using the IncTime procedure, increase the given time interval',0,5);
...
case v1 of
  1:
    begin
    s1 := 'PrevDate';
taskText('Using the MonthDays function from the task \1, write a procedure',0,1);
taskText('PrevDate({D}, {M}, {Y}) that changes a correct date, represented at the \<day~{D},',0,2);
taskText('month number~{M}, year~{Y}\> format, to a previous one ({D}, {M},~{Y} are input and',0,3);
taskText('output integer parameters). Apply this procedure to three given dates',0,4);
taskText('and output resulting previous ones.',0,5);
(*
*)
    end;
  2:
    begin
    s1 := 'NextDate';
taskText('Using the MonthDays function from the task \2, write a procedure',0,1);
taskText('NextDate({D}, {M}, {Y}) that changes a correct date, represented at the \<day~{D},',0,2);
taskText('month number~{M}, year~{Y}\> format, to a next one ({D}, {M},~{Y} are input and',0,3);
taskText('output integer parameters). Apply this procedure to three given dates',0,4);
taskText('and output resulting next ones.',0,5);
(*
*)
    end;
end;
...
begin
Topic1 := 'PROCEDURES WITH NUMERIC PARAMETERS';
