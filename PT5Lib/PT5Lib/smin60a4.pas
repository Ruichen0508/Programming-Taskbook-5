...
begin
taskText('Дано целое число~{N} и\ набор из\ {N}~целых чисел.',0,2);
taskText('Вывести в\ том\ же порядке номера всех нечетных чисел из\ данного набора',0,3);
taskText('и\ количество~{K} таких чисел.',0,4);
(*
*)
end;
...
taskText('число~0. Вывести номер первого числа в\ наборе, большего~{K}.',0,3);
taskText('Если таких чисел нет, то\ вывести~0.',0,4);
(*
*)
n := 2 + Random(8);
k := Random(7)+2;
...
taskText('Дано целое число~{K} и\ набор ненулевых целых чисел; признак его завершения\ \=',0,2);
taskText('число~0. Вывести номер последнего числа в\ наборе, большего~{K}.',0,3);
taskText('Если таких чисел нет, то\ вывести~0.',0,4);
(*
*)
n := 2 + Random(8);
...
taskText('образует убывающую последовательность, то\ вывести~0; в\ противном случае',0,3);
taskText('вывести номер первого числа, нарушающего закономерность.',0,4);
(*
*)
(*==*)
n := 2 + Random(8);
...
taskText('Если данный набор является пилообразным, то\ вывести~0; в\ противном случае',0,4);
taskText('вывести номер первого элемента, не\ являющегося зубцом.',0,5);
(*
*)
(*==*)
n := 3 + Random(7);
...
start(Name,Topic1,Author,Key,33{level2});
getVariant(10,0,0,0,v1,v2,v3);
taskText('Даны целые числа~{K}, {N} и\ набор из\ {N}~вещественных чисел:~{A}_{1}, {A}_{2},~\., {A}_{N}.',0,2);
taskText('Вывести {K}-e\ степени чисел из\ данного набора:',0,3);
taskText('\[({A}_{1})^K, ({A}_{2})^K,~\., ({A}_{N})^K.\]',0,4);
(*
*)
(*==*)
n := 2 + Random(6);
...
start(Name,Topic1,Author,Key,33{level2});
getVariant(1,0,0,0,v1,v2,v3);
taskText('Дано целое число~{N} и\ набор из\ {N}~вещественных чисел:~{A}_{1}, {A}_{2},~\., {A}_{N}.',0,2);
taskText('Вывести следующие числа:\[ {A}_{1}, ({A}_{2})^2,~\., ({A}_{N\-1})^{N\-1}, ({A}_{N})^N.\]',0,4);
(*
*)
(*==*)
n := 2 + Random(6);
...
getVariant(2,0,0,0,v1,v2,v3);
taskText('Дано целое число~{N} и\ набор из\ {N}~вещественных чисел:~{A}_{1}, {A}_{2},~\., {A}_{N}.',0,2);
taskText('Вывести следующие числа:\[ ({A}_{1})^N, ({A}_{2})^{N\-1},~\., ({A}_{N\-1})^2, {A}_{N}.\]',0,4);
(*
*)
(*==*)
n := 2 + Random(6);
...
taskText('Даны целые числа~{K}, {N},',0,2);
taskText('а\ также {K}~наборов целых чисел по\ {N}~элементов в\ каждом наборе.',0,3);
taskText('Для каждого набора вывести номер его первого элемента, равного~2,',0,4);
taskText('или число~0, если в\ данном наборе нет двоек.',0,5);
(*
*)
(*==*)
k := 1 + Random(5);
...
taskText('Даны целые числа~{K}, {N},',0,2);
taskText('а\ также {K}~наборов целых чисел по\ {N}~элементов в\ каждом наборе.',0,3);
taskText('Для каждого набора вывести номер его последнего элемента, равного~2,',0,4);
taskText('или число~0, если в\ данном наборе нет двоек.',0,5);
(*
*)
(*==*)
...
taskText('в\ противном случае вывести номер первого элемента, который не\ является зубцом.',0,5);
(*
*)
(*==*)
k := 2 + Random(4);
...
taskText('Дано целое число~{N} и\ набор из\ {N}~чисел.',0,2);
taskText('Найти номер минимального элемента из\ данного набора.',0,4);
(*
*)
n := 1 + Random(10);
a[1] := 19.98*Random - 10;
...
taskText('Вывести номер детали, изготовленной из\ материала максимальной плотности,',0,3);
...
resultN('Номер детали: ',nmax,xLeft,3,1);
...
start(Name,Topic3,Author,Key,43{level3});(*DEMO*)
getVariant(4,0,0,0,v1,v2,v3);
taskText('Дано целое число~{N} и\ набор из\ {N}~целых чисел. Найти номера',0,2);
...
for i := 1 to n do
  dataN('',b[i],center(i,n,1,6),4,1);
    resultN('Номер первого минимального элемента:     ',nmin,0,2,2);
    resultN('Номер последнего максимального элемента: ',nmax1,0,4,2);
setNumberOfTests(5);
pause;
end;
...
for i := 1 to n do
  dataN('',b[i],center(i,n,1,6),4,1);
    resultN('Номер первого максимального элемента:    ',nmax,0,2,2);
    resultN('Номер последнего минимального элемента:  ',nmin1,0,4,2);
setNumberOfTests(5);
Pause;
end;
...
s := 'мини';
taskText('Дано целое число~{N} и\ набор из\ {N}~целых чисел.',0,2);
taskText('Найти номера первого и\ последнего минимального элемента из\ данного набора',0,3);
taskText('и\ вывести\ их в\ указанном порядке.',0,4);
(*
*)
end;
...
s := 'макси';
taskText('Дано целое число~{N} и\ набор из\ {N}~целых чисел.',0,2);
taskText('Найти номера первого и\ последнего максимального элемента из\ данного набора',0,3);
taskText('и\ вывести\ их в\ указанном порядке.',0,4);
(*
*)
end;
...
for i := 1 to n do
  dataN('',b[i],center(i,n,1,6),4,1);
resultN('Номер первого '+s+'мального элемента:    ',nmax,0,2,2);
resultN('Номер последнего '+s+'мального элемента: ',nmax1,0,4,2);
setNumberOfTests(5);
Pause;
end;
...
begin
taskText('Дано целое число~{N} и\ набор из\ {N}~целых чисел.',0,2);
taskText('Найти номер первого \Iэкстремального\i (т.\,е. минимального',0,3);
...
begin
taskText('Дано целое число~{N} и\ набор из\ {N}~целых чисел.',0,2);
taskText('Найти номер последнего \Iэкстремального\i (т.\,е. минимального',0,3);
taskText('или максимального) элемента из\ данного набора.',0,4);
(*
*)
end;
...
getVariant(8,0,0,0,v1,v2,v3);
taskText('Дано целое число~{N} и\ набор из\ {N}~целых чисел.',0,2);
taskText('Найти номер первого максимального нечетного числа из\ данного набора.',0,3);
taskText('Если нечетные числа в\ наборе отсутствуют, то\ вывести~0.',0,4);
(*
+
*)
(*==*)
...
taskText('из\ тех элементов набора, которые больше~{B}, а\ также его номер.',0,3);
taskText('Если чисел, больших~{B}, в\ наборе нет, то\ дважды вывести~0.',0,4);
(*
+
*)
end;
...
taskText('из\ элементов набора, содержащихся в\ интервале ({B},\,{C}), и\ его номер.',0,3);
taskText('Если требуемые числа в\ наборе отсутствуют, то\ дважды вывести~0.',0,4);
(*
*)
end;
end;
...
if aMax=x0 then begin aMax:=0; nmax := 0; end;
if aMin=y0 then begin aMin:=0; nmin := 0; end;
case v1 of
1:dataR('B = ',x0,0,2,4);
2: begin
...
resultN('Его номер: ',nmin,xRight,3,1);
end;
2: begin
resultR('Максимальный элемент: ',amax,xLeft,3,4);
resultN('Его номер: ',nmax,xRight,3,1);
end;
end;
setNumberOfTests(5);
Pause;
...
getVariant(5,0,0,0,v1,v2,v3);
taskText('Дано целое число~{N} (>\,1) и\ набор из\ {N}~чисел.',0,2);
taskText('Найти номера двух соседних чисел из\ данного набора, произведение которых',0,3);
taskText('является минимальным, и\ вывести вначале меньший, а\ затем больший номер.',0,4);
(*
*)
(*==*)
n := 2 + Random(9);
...
for i := 1 to n do
  dataR('',a[i],center(i,n,5,2),4,5);
resultN('Меньший номер: ',nmin,0,2,1);
resultN('Больший номер: ',nmin+1,0,4,1);
setNumberOfTests(5);
Pause;
end;
...
taskText('нули и\ единицы. Найти номер элемента, с\ которого начинается',0,2);
...
taskText('то\ вывести номер первой из\ них.',0,5);
(*
*)
end;
2:
begin
taskText('Дано целое число~{N} и\ набор из\ {N}~целых чисел, содержащий только',0,1);
taskText('нули и\ единицы. Найти номер элемента, с\ которого начинается самая длинная',0,2);
taskText('последовательность единиц, и\ количество элементов в\ этой последовательности.',0,3);
taskText('Если таких последовательностей несколько, то\ вывести номер последней из\ них.',0,4);
taskText('Если единицы в\ исходном наборе отсутствуют, то\ дважды вывести~0.',0,5);
(*
*)
end;
end;
...
resultN('Номер начального элемента: ',nMax,xLeft,3,1);
