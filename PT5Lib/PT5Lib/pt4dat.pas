...
type TSampleLines = array [0..MaxLineCount - 1] of string;

procedure SampleTextEn(n: integer; var LineCount: integer; var Lines: TSampleLines);
var i,j,k: integer;
    s0: string;
begin
  LineCount := -1;
  for i := 0 to MaxLineCount-1 do
    Lines[i] := SampleError;
  if (n<0) or (n>= NS0En) then exit;
  j := n + 1;
  LineCount := begEn_[j+1] - begEn_[j];
  k := -1;
    for i := 0 to LineCount - 1 do
      begin
        Inc(k);
        if i = 0 then s0 := Copy(SEn_[begEn_[j]+i],2,60)
        else
          begin
            s0 := SEn_[begEn_[j]+i];
...
var LineCount, i: integer;
    ...
    s: string;
begin
  SampleTextEn(N, LineCount, Lines);
  s := Lines[0];
