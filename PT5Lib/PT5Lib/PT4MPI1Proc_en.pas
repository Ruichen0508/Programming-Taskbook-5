...
const DebugSec = 'The content of the debug section should be as follows:';
      Title1 = 'Processes and their ranks';
      Process1 = 'Process ';
...
begin
  ...
  if n = 0 then exit;
  TaskText(
'Input a real number {X} in each process of the MPI\_COMM\_WORLD communicator'#13+
'and output its opposite value \-{X}. For data input and output,'#13+
'use the input-output stream pt. Also output the value \-{X} in the debug'#13+
...
*)
  ...
  begin
    SetProcess(i);
    d := Random*199.99-100;
    DataR(Process1+Chr(i+48)+': ', d, 0, y+i, 6);
    ResultR(Process1+Chr(i+48)+': ', -d, 0, y+i, 6);
    Str(-d:0:2, s);
...
begin
  ...
  if n = 0 then exit;
  TaskText(
'Input an integer {A} in each process of the MPI\_COMM\_WORLD communicator'#13+
'and output doubled value of {A}. Also output the total number'#13+
'of processes in the \Imaster process\i (that is, a rank-zero process).'#13+
'For data input and output use the input-output stream pt.'#13+
'In the master process, duplicate the data output in the debug section'#13+
'by displaying on separate lines the doubled value of {A} and'#13+
'the total number of processes (use two calls of the ShowLine function,'#13+
...
*)
  ...
  begin
    SetProcess(i);
    d := Random(40)+10;
    DataN(Process1+Chr(i+48)+':  A = ', d, 0, y+i, 2);
    if i > 0 then
    ResultN(Process1+Chr(i+48)+': ', 2*d, 0, y+i, 2)
    else
    begin
    d0 := d;
    ResultN2(Process1+Chr(i+48)+': ', 2*d0, n, 34, y+i, 2);
    end;
  end;
...
begin
  ...
  if n = 0 then exit;
  TaskText(
'Input a real number {X} and output its opposite value in the master process.'#13+
'Also output the rank of \Islave processes\i (which are processes whose rank'#13+
'is greater than~0); the rank of each process should be output'#13+
'in the process of this rank. In addition, duplicate the data output'#13+
'in the debug section by displaying the value of \-{X}'#13+
'with the \<-X=\> comment and the rank values with the \<rank=\> comments'#13+
'(use the Show function calls with two parameters).'
  );
...
*)
  ...
  begin
    ...
    if i > 0 then
    begin
    ResultN(Process1+Chr(i+48)+': ', i, 0, y+i, 6);
    DataComment(' '+IntToStr(i)+'|  1>  rank='+IntToStr(i), 1, 3+i);
    end
    else
    begin
      d := Random*199.99-100;
      DataR(Process1+Chr(i+48)+':  X = ', d, 0, 1, 6);
      ResultR(Process1+Chr(i+48)+': ', -d, 0, y+i, 6);
      Str(-d:0:2, s);
...
begin
  ...
  if n = 0 then exit;
  TaskText(
'Input one integer in processes with even rank (inclusive'#13+
'of the master process) and output the doubled value of input number.'#13+
...
*)
  ...
  begin
    SetProcess(2*i);
    d := Random(40)+10;
    DataN(Process1+Chr(2*i+48)+': ', d, 0, y+i, 2);
    ResultN(Process1+Chr(2*i+48)+': ', 2*d, 0, y+i, 2);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  if n = 0 then exit;
    TaskText(
'Input one integer in processes with even rank (including the master process),'#13+
'input one real number in processes with odd rank. Output doubled value'#13+
'of input number in each process.'
  );
...
*)
  ...
  begin
    SetProcess(2*i);
    d := Random(40)+10;
    DataN(Process1+Chr(2*i+48)+': ', d, xLeft, y+i, 2);
    ResultN(Process1+Chr(2*i+48)+': ', 2*d, xLeft, y+i, 2);
  end;
  for i := 1 to n div 2 do
  begin
    SetProcess(2*i-1);
    r := 39.98*Random+10;
    DataR(Process1+Chr(2*i-1+48)+': ', r, xRight, y+i-1, 5);
    ResultR(Process1+Chr(2*i-1+48)+': ', 2*r, xRight, y+i-1, 5);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  if n = 0 then exit;
  TaskText(
'Input one integer in slave processes with even rank, input one real number'#13+
'in processes with odd rank. Output doubled value of input number in each slave'#13+
'process. Do not perform any action in the master process.'
  );
(*
...
*)
  ...
  begin
    ...
    if (n-1) mod 4 <> 0 then
      y1 := y1 - 1;
    DataN(Process1+Chr(2*i+48)+': ', d, xRight, y1, 2);
    ResultN(Process1+Chr(2*i+48)+': ', 2*d, xRight, y1, 2);
  end;
  for i := 1 to n div 2 do
  begin
    SetProcess(2*i-1);
    ...
    if (n-1) mod 4 <> 0 then
      y1 := y1 - 1;
    DataR(Process1+Chr(2*i-1+48)+': ', r, xLeft, y1, 5);
    ResultR(Process1+Chr(2*i-1+48)+': ', 2*r, xLeft, y1, 5);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  if n = 0 then exit;
    TaskText(
'An integer~{N} (>\,0) and a sequence of {N}~real numbers are given'#13+
'in each process with even rank (including the master process).'#13+
'Output the sum of given numbers in each process.'#13+
'Do not perform any action in processes with odd rank.'
  );
...
*)
  ...
  begin
    ...
    k := 1 + Random(4);
    s := 0;
    DataN(Process1+Chr(2*i+48)+': N = ', k, 21, y+i, 1);
    for j := 1 to k do
    begin
    r := Random*9.95;
    s := s + r;
    ...
    ResultR(Process1+Chr(2*i+48)+': ', s, 0, y+i, 5);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  TaskText(
'An integer~{N} (>\,0) and a sequence of {N}~real numbers are given'#13+
'in each process. Output the sum of given numbers in each process'#13+
'with even rank (including the master process), output the average'#13+
'of given numbers in each process with odd rank.'
  );
...
*)
  ...
  begin
    ...
    k := 1 + Random(4);
    s := 0;
    DataN(Process1+Chr(2*i+48)+': N = ', k, 3, y+i, 1);
    for j := 1 to k do
    begin
    r := Random*9.95;
    s := s + r;
    ...
    ResultR(Process1+Chr(2*i+48)+': ', s, 14, y+i, 5);
  end;
  for i := 1 to n div 2 do
  begin
    SetProcess(2*i-1);
    k := 1 + Random(4);
    s := 0;
    DataN(Process1+Chr(2*i-1+48)+': N = ', k, 41, y+i-1, 1);
    for j := 1 to k do
    begin
    r := Random*9.95;
    s := s + r;
    ...
    ResultR(Process1+Chr(2*i-1+48)+': ', s/k, 52, y+i-1, 1);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  TaskText(
'An integer~{N} (>\,0) and a sequence of {N}~real numbers are given'#13+
'in each process. Output the sum of given numbers in each slave process'#13+
'with even rank, output the average of given numbers in each process'#13+
'with odd rank, output the product of given numbers in the master process.'
  );
...
*)
  ...
  begin
    ...
    ResultR(Process1+Chr(2*i+48)+': ', s, 14, y+i, 7);
  end;
  for i := 1 to n div 2 do
  begin
    SetProcess(2*i-1);
...
begin
  ...
  if n = 0 then exit;
  TaskText(
'An integer~{N} (>\,0) and a sequence of {N} numbers are given'#13+
'in each process. The sequence contains real numbers in the slave processes'#13+
'with odd rank (1, 3,~\.) and integers in the slave processes with even rank'#13+
'(2, 4~\.). The type of elements in the master process depends on the number'#13+
'of processes {K}: if {K} is an odd number then the sequence contains integers'#13+
'otherwise the sequence contains real numbers. Output the minimal'#13+
'element of the given sequence in each even-rank process (including'#13+
'the master process), output the maximal element of the given sequence'#13+
'in each odd-rank process.'
  );
...
*)
  ...
  begin
    ...
    if (i > 0) or Odd(n) then
      ResultN(Process1+Chr(2*i+48)+': ', s0, 14, y+i, 4)
    else
      ResultR(Process1+Chr(2*i+48)+': ', s, 14, y+i, 3);
  end;
  for i := 1 to n div 2 do
  begin
    SetProcess(2*i-1);
    ...
    ResultR(Process1+Chr(2*i-1+48)+': ', s, 52, y+i-1, 1);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(i);
    d := Random(90)+10;
    DataN(Process1+Chr(i+48)+': ', d, 0, y+i, 2);
    SetProcess(0);
...
begin
  ...
  begin
    SetProcess(n-i);
    d := Random*89.99+10;
    DataR(Process1+Chr(n-i+48)+': ', d, 0, y+n-i, 5);
    SetProcess(0);
...
begin
  ...
  for i := 1 to n-1 do
  begin
    DataComment(Process1+Chr(i+48)+': ',30,y+i);
    for j := 1 to 4 do
    begin
      inc(c);
      d := Random(90)+10;
...
begin
  ...
  begin
    SetProcess(i);
    DataN(Process1+Chr(i+48)+': N = ',k[i], 23, y+i, 1);
    for j := 1 to k[i] do
    begin
      inc(c);
      d := Random(90)+10;
...
begin
  ...
  begin
    SetProcess(i);
    d := Random*89.99+10;
    ResultR(Process1+Chr(i+48)+': ', d, 0, y+i, 5);
    SetProcess(0);
...
begin
  ...
  begin
    SetProcess(n-i);
    d := Random*89.99+10;
    ResultR(Process1+Chr(n-i+48)+': ', d, 0, y+n-i, 5);
    SetProcess(0);
    DataR('', d, Center(i,n-1,5,2), 3, 5)
  end;
  SetTestCount(5);
...
begin
  ...
  begin
    ...
    if i = c then
      ...
      d1 := 0;
    DataN(Process1+Chr(i+48)+': ', d1, 0, y+i, 2)
  end;
  SetProcess(0);
...
begin
  ...
  begin
    ...
    if i = c then
      ...
      d1 := 0;
    DataN(Process1+Chr(i+48)+': N = ', d1, 2, y+i, 1);
    if i = c then
    for j := 1 to d do
    begin
      r := Random*89.99+10;
...
begin
  ...
  begin
    SetProcess(i);
    DataN(Process1+Chr(i+48)+': N = ', a[i], xRight, y+i, 2);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(i);
    DataR(Process1+Chr(i+48)+': ', a[i], xRight, y+i, 2);
    ResultR(Process1+Chr(i+48)+': ', a[0], xRight, y+i, 2);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    ...
    c := (i+1) mod n;
    SetProcess(c);
    ResultN(Process1+Chr(c+48)+': ', d, 0, y+c, 2);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(i);
    DataN2(Process1+Chr(i+48)+':', a[i], b[i], 0, y+i, 3);
    ResultN2(Process1+Chr(i+48)+':', b[(i+n-1) mod n], a[(i+1) mod n], 0, y+i, 3);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    r := Random*89.99+10;
    SetProcess(i);
    DataR(Process1+Chr(i+48)+': A = ', r, 26, y+i, 5);
    DataN('N = ',b[i],48,y+i, 1);
    SetProcess(b[i]);
    ResultR(Process1+Chr(b[i]+48)+': ', r, 30, y+b[i], 5);
...
begin
  ...
  begin
    ...
    if i <> c then
      ...
      k := 1;
    DataN(Process1+Chr(i+48)+': N = ', k, 0, y+i, 1);
    if i = c then
      for j := 0 to n-2 do
  ...
  begin
    ...
    SetProcess(i);
    ResultR(Process1+Chr(i+48)+': ', a[i-j], 0, y+i-j, 5);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    ...
    ResultComment(Process1+Chr(i+48)+':', 30, y+i);
    k := -1;
    for j := 0 to n-2 do
    begin
      k := k + 1;
...
begin
  ...
  begin
    SetProcess(2*i);
    DataN(Process1+Chr(2*i+48)+': N = ', k[2*i], 3, y+i, 1);
    for j := 1 to k[2*i] do
      DataR('',a[2*i,j],16+j*5,y+i,4);
    ResultComment(Process1+Chr(2*i+48)+': ', 3, y+i);
  ...
  begin
    SetProcess(2*i-1);
    DataN(Process1+Chr(2*i-1+48)+': N = ', k[2*i-1], 41, y+i-1, 1);
    for j := 1 to k[2*i-1] do
      DataR('',a[2*i-1,j],54+j*5,y+i-1,4);
    ResultComment(Process1+Chr(2*i-1+48)+': ', 41, y+i-1);
...
begin
  ...
  begin
    SetProcess(2*i);
    DataR(Process1+Chr(2*i+48)+': ', a[2*i], xLeft, y+i, 5);
    ResultR(Process1+Chr(2*i+48)+': ', a[n-1-2*i], xLeft, y+i, 5);
  end;
  for i := 1 to n div 2 do
  begin
    SetProcess(2*i-1);
    DataR(Process1+Chr(2*i-1+48)+': ', a[2*i-1], xRight, y+i-1, 5);
    ResultR(Process1+Chr(2*i-1+48)+': ', a[n-2*i], xRight, y+i-1, 5);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(i);
    DataR(Process1+Chr(i+48)+': A = ', d[i], 27, y+i, 5);
...
begin
  ...
  begin
    SetProcess(i);
    DataN(Process1+Chr(i+48)+': L = ', l[i], 3, y+i, 1);
    for j := 1 to l[i] do
    begin
    m := m + 1;
...
begin
  ...
  begin
    SetProcess(i);
    case t[i] of
    0: ResultN(Process1+Chr(i+48)+': ', b[i], 32, y+i, 2);
    1: ResultR(Process1+Chr(i+48)+': ', a[i], 0, y+i, 5);
    end;
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(i);
    DataN(Process1+Chr(i+48)+': T = ', t[i], 3, y+i, 1);
...
begin
  ...
  begin
    SetProcess(i);
    ResultN(Process1+Chr(i+48)+': ', k, 0, y+i, 2);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+': ',13,y+i);
    for j := 1 to 5 do
...
begin
  ...
  begin
    SetProcess(i);
    DataR(Process1+Chr(i+48)+': ', r[i+1], 0, y+i, 5);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(i);
    DataComment(Process1+Chr(i+48)+': ',22,y+i);
    for j := 1 to 5 do
...
begin
  ...
  begin
    SetProcess(i);
    DataComment(Process1+Chr(i+48)+': ',20,y+i);
    for j := 1 to i+2 do
...
begin
  ...
  begin
    SetProcess(i);
    ResultR(Process1+Chr(i+48)+': ', a[i+1], 0, y+i, 5);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+': ',21,y+i);
    for j := 1 to 3 do
...
begin
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+': ',20,y+i);
    for j := 1 to i+2 do
...
begin
  ...
  begin
    SetProcess(i);
    DataR(Process1+Chr(i+48)+': ', a[i+1], 0, y+i, 5);
  end;
  for i := 0 to n - 1 do
  begin
    SetProcess(i);
...
begin
  ...
  begin
    SetProcess(i);
    DataComment(Process1+Chr(i+48)+': ',24,y+i);
    for j := 1 to 4 do
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+': ',5,y+i);
    for k := 1 to n do
      for j := 1 to 4 do
...
begin
  ...
  begin
    SetProcess(i);
    DataComment(Process1+Chr(i+48)+': ',13,y+i);
    for j := 1 to n do
...
begin
  ...
  begin
    SetProcess(i);
    DataComment(Process1+Chr(i+48)+': ',7,y+i);
    for j := 1 to 3*n do
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+': ',7,y+i);
    c0 := 0;
    for j := 1 to n do
      for k := 1 to 3 do
      begin
...
begin
  ...
  begin
    SetProcess(i);
    DataComment(Process1+Chr(i+48)+': ',15,y+i);
    for j := 1 to c do
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+': ',15,y+i);
    c0 := 0;
    for j := 1 to n do
      for k := 1 to i+1 do
      begin
...
begin
  ...
  begin
    SetProcess(i);
    DataComment(Process1+Chr(i+48)+': ',5,y+i);
    for j := 1 to c do
...
begin
  ...
  for i := 2 to n do
    for j := 1 to c do
      a[1,j] := a[1,j] + a[i,j];
    SetProcess(0);
    ResultComment(Process1+Chr(0+48)+':',0,2);
    for j := 1 to c do
...
begin
  ...
  begin
    SetProcess(i);
    DataComment(Process1+Chr(i+48)+': ',6,y+i);
    for j := 1 to c do
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+': ',6,y+i);
    for j := 1 to c do
...
begin
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+':',22,y+i);
...
begin
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+':',25,y+i);
...
begin
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+': ',3,y+i);
    for j := 1 to n-1 do
...
begin
  ...
  begin
    SetProcess(i);
    DataComment(Process1+Chr(i+48)+': ',3,y+i);
...
begin
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+': ',24,y+i);
...
begin
  ...
  begin
    SetProcess(i);
    ResultComment(Process1+Chr(i+48)+': ',9,y+i);
    for j := 1 to n do
...
begin
  ...
  begin
    k := 2*i-2;
    SetProcess(k);
    ResultN(Process1+Chr(k+48)+': ', a[i], 0, y+i, 2);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    k := 2*i-1;
    SetProcess(k);
    DataR2(Process1+Chr(k+48)+':', a[k], a[k+1], 0, y+i, 6);
  end;
  for i := 1 to n1 do
  begin
    k := 2*i-1;
    SetProcess(k);
    ResultComment(Process1+Chr(k+48)+':',5,y+i);
    for j := 1 to 2*n1 do
...
begin
  ...
  begin
    ...
    SetProcess(k);
    Str(k:j,s);
    DataN3(Process1+s+':', a[k+1], a[k+2], a[k+3], 0, y+i, 3);
  end;
    SetProcess(0);
    ResultComment('Процесс 0:',0,2);
    for j := 1 to 3*n1 do
...
begin
  ...
  begin
    k := 2*i-2;
    SetProcess(k);
    DataR3(Process1+Chr(k+48)+':', a[i,1], a[i,2], a[i,3], 0, y+i, 6);
  end;
    SetProcess(0);
    ResultComment(' Процесс 0:',0,2);
    for j := 1 to 3 do
...
begin
  ...
  begin
    SetProcess(2*i);
    r := 89.98*Random+10;
    DataR(Process1+Chr(2*i+48)+': ', r, xLeft, y+i, 5);
    if r > max then
      max := r;
  end;
  for i := 1 to n div 2 do
...
//  if (n+1) div 2 = 5 then y := 1;
  ...
  begin
    SetProcess(2*i-1);
    DataN(Process1+Chr(2*i-1+48)+': N = ', b[2*i-1], xLeft, y+i-1, 1);
  end;
  for i := 1 to (n+1) div 2 - 1 do
  begin
    SetProcess(2*i);
    DataN(Process1+Chr(2*i+48)+': N = ', b[2*i], xRight, y+i-1, 1)
  end;
  y := 1;
  if k = 2 then y := 2;
  if k = 5 then y := 0;
  ...
  begin
    ...
    begin
      j := j + 1;
      SetProcess(i);
      ResultR(Process1+Chr(i+48)+': ', r[j], 0, y+j, 5);
    end;
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(2*i);
    DataN(Process1+Chr(2*i+48)+': N = ', b[2*i], 5, y+i, 1);
    if b[2*i]=1 then
  ...
  begin
    SetProcess(2*i-1);
    DataN(Process1+Chr(2*i-1+48)+': N = ', b[2*i-1], 45, y+i-1, 1);
    if b[2*i-1]=1 then
      DataR('A = ', r1[2*i-1], 63, y+i-1, 5);
  end;
    SetProcess(m);
    ResultComment(Process1+Chr(m+48)+':', 5, 3);
    for i := 1 to k do
...
begin
  ...
  for m := 0 to n-1 do
    ...
    begin
      j := j + 1;
      SetProcess(m);
      ResultComment(Process1+Chr(m+48)+':', 5, y+j);
      for i := 1 to k do
...
begin
  ...
  begin
    SetProcess(2*i);
    ResultComment(Process1+Chr(2*i+48)+':', 5, y+i);
    if b[2*i]=1 then
      for m := 1 to k do
  ...
  begin
    SetProcess(2*i-1);
    ResultComment(Process1+Chr(2*i-1+48)+':', 45, y+i-1);
    if b[2*i-1]=1 then
      for m := 1 to k do
...
begin
  ...
  for m := 0 to n-1 do
    ...
    begin
      j := j + 1;
      SetProcess(m);
      ResultR(Process1+Chr(m+48)+': ', sum, 5, y+j, 6);
    end;
  SetTestCount(5);
end;
...
begin
  ...
  for i := 1 to k do
    ...
    begin
      m := m + 1;
      SetProcess(m);
      ResultN2(Process1 + Chr(m+48)+':', i-1,j-1,Center(j,n div k,14,d),y+i,2);
    end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    ...
    r[i] := 8.98*Random+1;
    DataR(Process1+Chr((i-1)*(n div k)+48)+': A = ',r[i],0,2*i,3);
  end;
  y := 1;
  if k = 5 then y := 0;
  if k = 2 then y := 2;
  ...
  for i := 1 to k do
    ...
    begin
      m := m + 1;
      SetProcess(m);
      ResultR(Process1 + Chr(m+48)+': ', r[i],Center(j,n div k,14,d),y+i,3);
    end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(i-1);
    r[i] := 8.98*Random+1;
    DataR(Process1+Chr(i-1+48)+': A = ',r[i],0,2*i,3);
  end;
  y := 1;
  ...
  for i := 1 to n div k do
    ...
    begin
      m := m + 1;
      SetProcess(m);
      ResultR(Process1 + Chr(m+48)+': ', r[j],Center(j,k,14,d),y+i,3);
    end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess((i-1)*n);
    DataComment(Process1+Chr((i-1)*n+48)+':',22,i+1);
    r[i,j] := Random(90) + 10;
  ...
  for i := 1 to k do
    ...
    begin
      ...
      SetProcess(m);
      str(m:2,s);
      ResultN(Process1 + s+': ', r[i,j],Center(j,n,13,d),y+i,2);
    end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(i-1);
    DataComment(Process1+Chr(i-1+48)+':',22,i+1);
    r[i,j] := Random(90) + 10;
    DataN('',r[i,j],Center(j,n,2,2),i+1,2);
  end;
  y := 1;
  ...
  for i := 1 to n do
    ...
    begin
      ...
      SetProcess(m);
      str(m:2,s);
      ResultN(Process1 + s+': ', r[j,i],Center(j,k,13,d),y+i,2);
    end;
  SetTestCount(5);
end;
  ...
  for i := 1 to 2 do
    for j := 1 to k do
      begin
        ...
        DataN(Process1+s+': ',r[i,l,j],Center(j,k,14,4),i+(l-1)*3,2);
      end;
  m := 0;
  y := 1;
  for l := 1 to 2 do
...
begin
  ...
  begin
    SetProcess(j-1);
    str(j-1,s);
    ResultComment(Process1 + s+':', 20, y+j);
    m := 0;
    for l := 1 to 2 do
    for i := 1 to 2 do
    begin
...
begin
  ...
  for i := 1 to 2 do
    for j := 1 to k do
      begin
        ...
        DataR(Process1+s+': ',r[i,l,j],Center(j,k,17,4),i+(l-1)*3,4);
      end;
  for i := 1 to 2 do
  for j := 1 to k do
  ...
  begin
    ...
    begin
      SetProcess(m);
      str(m,s);
      ResultR(Process1 + s+': ', r1[i,j],Center(j,k,17,4),y+2*i,5);
      m := m + 1;
    end;
  end;
  SetTestCount(5);
  ...
  for i := 1 to 2 do
    for j := 1 to k do
      begin
        ...
        str(m:2,s);
        r[i,l,j] := 8.98*Random+1;
        DataR(Process1+s+': ',r[i,l,j],Center(j,k,17,5),i+(l-1)*3,5);
      end;
  for j := 1 to k do
  begin
    r1[j] := 0;
  ...
  for i := 1 to 2 do
    for j := 1 to k do
      begin
        ...
        str(m:2,s);
        r[i,l,j] := 8.98*Random+1;
        ResultR(Process1+s+': ',r1[j],Center(j,k,17,5),i+(l-1)*3,5);
      end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(2*i);
    x := 16;
    DataComment(Process1+Chr(2*i+48)+': ', 5, y+i);
    if i=0 then
    begin
    DataN('M =', m, x, y+i, 2);
    DataN('N =', k, x+7, y+i, 2);
  ...
  begin
    SetProcess(2*i-1);
    DataComment(Process1+Chr(2*i-1+48)+':', 45, y+i-1);
    x := 56;
  ...
  begin
    SetProcess(2*i);
    ResultN(Process1+Chr(2*i+48)+': ', GetRank(m,k,a[2*i],b[2*i],1,0), 5, y+i, 2);
  end;
  for i := 1 to n div 2 do
  begin
    SetProcess(2*i-1);
    ResultN(Process1+Chr(2*i-1+48)+': ', GetRank(m,k,a[2*i-1],b[2*i-1],1,0), 45, y+i-1, 2);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(2*i);
    ResultN(Process1+Chr(2*i+48)+': ', GetRank(m,k,a[2*i],b[2*i],0,1), 5, y+i, 2);
  end;
  for i := 1 to n div 2 do
  begin
    SetProcess(2*i-1);
    ResultN(Process1+Chr(2*i-1+48)+': ', GetRank(m,k,a[2*i-1],b[2*i-1],0,1), 45, y+i-1, 2);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(i);
    DataR(Process1+Chr(i+48)+': ', a[i], 0, y+i, 5);
  end;
  for i := 1 to n - 1 do
  begin
    SetProcess(i-1);
    ResultR(Process1+Chr(i-1+48)+': ', a[i], 0, y+i, 5);
  end;
  SetTestCount(5);
end;
...
begin
  ...
  for i := 1 to k do
    ...
    begin
      m := m + 1;
      SetProcess(m);
      ResultR(Process1 + Chr(m+48)+': ', r[i,j-1],Center(j,n div k,14,d),y+i,3);
    end;
  m := -1;
  for i := 1 to k do
    for j := 1 to n div k do
    begin
      m := m + 1;
      SetProcess(m);
      DataR(Process1 + Chr(m+48)+': ', r[i,j mod (n div k)],Center(j,n div k,14,d),y+i,3);
    end;
  SetTestCount(5);
end;
  ...
  for i := 1 to 2 do
    for j := 1 to k do
      begin
        ...
        j1 := j - 1;
        if j1 = 0 then j1 := k;
        ResultR(Process1+s+': ',r[i,l,j1],Center(j,k,17,5),i+(l-1)*3,5);
      end;
  SetTestCount(5);
end;
...
begin
  ...
  begin
    SetProcess(2*i-1);
    DataN(Process1+Chr(2*i-1+48)+': A = ', a[2*i-1], xLeft, y+i, 2);
  end;
  for i := 1 to (n+1) div 2 - 1 do
  begin
    SetProcess(2*i);
    DataN(Process1+Chr(2*i+48)+': A = ', a[2*i], xRight, y+i, 2)
  end;
    SetProcess(0);
  ...
  begin
    SetProcess(2*i-1);
    ResultComment(Process1+Chr(2*i-1+48)+':',12,y+i);
  ...
  begin
    SetProcess(2*i);
    ResultComment(Process1+Chr(2*i+48)+':',51,y+i);
...
begin
  ...
  begin
    SetProcess(2*i);
    DataN(Process1+Chr(2*i+48)+': A = ', a[2*i], xLeft, y+i, 2);
  end;
  for i := 1 to n div 2 do
  begin
    SetProcess(2*i-1);
    DataN(Process1+Chr(2*i-1+48)+': A = ', a[2*i-1], xRight, y+i-1, 2);
  end;

  for i := 0 to (n+1) div 2 - 1 do
  begin
    SetProcess(2*i);
    ResultComment(Process1+Chr(2*i+48)+':', 12, y+i);
    j := 23;
    if 2*i-2 >=0 then
    begin
  ...
  begin
    SetProcess(2*i-1);
    ResultN(Process1+Chr(2*i-1+48)+': ', a[2*i-2], 51, y+i-1, 2);
  end;
  SetTestCount(5);
end;                                 
...
begin
  ...
  begin
    ...
    SetProcess(m);
    Str(m:2,s);
    DataN(Process1+s+': A = ', a[m], Center(j,3,18,6), y+i, 2);
  end;
  SetProcess(0);
  ...
  begin
    ...
    SetProcess(m);
    Str(m:2,s);
    ResultComment(Process1+s+':', -17+j*24, y+i);
    case j of
    1: begin
...
begin
  ...
  begin
    ...
    d := Random*199.99-100;
    Str(i:2, s);
    DataR(Process1+s+': ', d, 0, i+1, 6);
    ResultR(Process1+s+': ', -d, 0, i+1, 6);
  end;
  SetTestCount(5);
end;
*)
...
begin
  ...
  CreateGroup('MPI1Proc', Title1, 'M. E. Abramyan, 2014,2017', 'sddfgfsjnbvvdfsd78fgfgd', 10, InitTask);
