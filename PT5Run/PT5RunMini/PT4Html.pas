...
//2015.03G>
       'G': // picture loading
         begin
           ...
           {$IFDEF WINDOWS}
           ccomm := ccomm + Parfinal
             + '<p class="ptPicture"><img src="'+picname+'" alt="'+picname+'" border=1></p>';
           {$ENDIF}
           {$IFDEF LINUX}
           ccomm := ccomm + Parfinal + '<p class="ptPicture"><img src="' + picname + '" alt="' + picname + '" border=1></p>';
           {$ENDIF}
           if i>=length(c) then begin ParFinal := ''; continue; end;
...
begin
  ...
  DN[0] := #0;
  RES[0] := #0;
  Command := 'xdg-mime query default application/' + ExtractFileExt(FN);//FindExecutable(FN, DN, RES);
  Result := StrPas(RES);
end;
...
//  else
  ...
  {$ENDIF}
  {$IFDEF LINUX}
  proc.Executable := '/usr/bin/xdg-open';
  {$ENDIF}
