...
//2024.04>
  s0 := '';  
  if not forHeader then // для заголовка формулировки указываем ТОЛЬКО название группы/подгруппы, 
                        // без имени задания и без квадратных скобок,
                        // чтобы можно было использовать боле длинное название!
  begin
//    s0 := s0 + ' [' ;
    s3 := GrDescr;
    if SubgrDescr <> '' then
